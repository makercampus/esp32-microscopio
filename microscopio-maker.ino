...
// Primary config, or defaults.
    ...
    stationList[] = {{MAKER_DEFAULT_AP_SSID, MAKER_DEFAULT_AP_PASSWORD, true}};
#endif
...
#else
    char myName[] = MAKER_BRAND_NAME;
#endif

#if defined(MDNS_NAME)
    char mdnsName[] = MDNS_NAME;
#else
    char mdnsName[] = MAKER_DEFAULT_MDNS_NAME;
#endif
...
// Illumination LAMP and status LED
    ...
    int lampVal = -1; // lamp is disabled in config
        ...
        int lampVal = constrain(LAMP_DEFAULT,0,maker::kLampMaxPercent);
    #else
...
int activeProfileIndex = maker::kDefaultProfileIndex;
...
#if defined(LAMP_PIN)
    if (newVal == -1) return;
    const int clampedLamp = constrain(newVal, 0, maker::kLampMaxPercent);
    // Apply a logarithmic function to the scale.
    const int brightness = round((pow(2, (1 + (clampedLamp * 0.02))) - 2) / 6 * pwmMax);
    ledcWrite(LAMP_PIN, brightness);
    Serial.print("Lamp: ");
    Serial.print(clampedLamp);
    Serial.print("%, pwm = ");
    Serial.println(brightness);
#endif
}

int clampLampPercent(int value) {
    if (value < 0) return 0;
    return constrain(value, 0, maker::kLampMaxPercent);
}

int profileIndexFromKey(const char *key) {
    if (!key || strlen(key) == 0) return -1;
    if (!strcmp(key, "low") || !strcmp(key, "bajo")) return 0;
    if (!strcmp(key, "medium") || !strcmp(key, "mediano")) return 1;
    if (!strcmp(key, "high") || !strcmp(key, "alto")) return 2;
    return -1;
}

const char *getProfileKeyByIndex(int index) {
    if (index < 0 || index >= static_cast<int>(maker::kProfileCount)) return "custom";
    return maker::kProfiles[index].key;
}

const char *getProfileLabelByIndex(int index) {
    if (index < 0 || index >= static_cast<int>(maker::kProfileCount)) return "Personalizado";
    return maker::kProfiles[index].label;
}
...
bool applyProfileByIndex(int index) {
    if (index < 0 || index >= static_cast<int>(maker::kProfileCount)) return false;

    sensor_t *s = esp_camera_sensor_get();
    if (!s) return false;

    const maker::CameraProfileSetting &profile = maker::kProfiles[index];
    const int qualityRaw = maker::qualityPercentToSensorRaw(profile.qualityPercent, sensorPID);

    int res = 0;
    if (s->pixformat == PIXFORMAT_JPEG) {
        res |= s->set_framesize(s, profile.frameSize);
    }
    res |= s->set_quality(s, qualityRaw);
    res |= s->set_xclk(s, LEDC_TIMER_0, profile.xclkMHz);

    if (res != 0) return false;

    xclk = profile.xclkMHz;
    activeProfileIndex = index;

    Serial.printf("Perfil aplicado: %s, frame=%d, quality=%d (%d%%), xclk=%luMHz\r\n",
                  profile.label, profile.frameSize, qualityRaw, profile.qualityPercent, xclk);
    return true;
}
...
void StartCamera() {
    ...
    if (err != ESP_OK) {
        ...
        critERR += "<p>Si el problema persiste, verifica conexiones y alimentación.</p>";
    } else {
        ...
        if (!applyProfileByIndex(maker::kDefaultProfileIndex)) {
            Serial.println("No se pudo aplicar el perfil por defecto. Se usan valores base del sensor.");
...
void setup() {
    ...
    Serial.println();
    Serial.println("====");
    Serial.print("maker-campus-microscopio: ");
    ...
    */
    ...
    if (otaEnabled) {
        ...
        ArduinoOTA
            .onStart([]() {
                ...
                critERR += "<p>Wait for OTA to finish and then reinicia el módulo manualmente.</p>";
            })
            .onEnd([]() {
                Serial.println("\r\nEnd");
            })
    ...
    if (lampVal != -1) {
        #if defined(LAMP_PIN)
            if (!ledcAttach(LAMP_PIN, pwmfreq, pwmresolution)) {
                Serial.println("Error configurando PWM de la lámpara");
            } else {
                lampVal = clampLampPercent(lampVal);
                setLamp(lampVal);
            }
         #endif
    } else {
